...
static enum Detector{ HARRIS, SHITOMASI, FAST, BRISK, ORB, AKAZE, SIFT};
static enum Descriptor{BRISK, BRIEF, ORB, FREAK, AKAZE, SIFT};

static map<string, Detector> mapDetector;
static map<string, Detector> mapDescriptor;

void Detector_Map()
{
  mapDetector["HARRIS"] = HARRIS;
  mapDetector["SHITOMASI"] = SHITOMASI;
  mapDetector["BRISK"] = BRISK;
  mapDetector["ORB"] = ORB;
  mapDetector["AKAZE"] = AKAZE;
  mapDetector["SIFT"] = SIFT;
}

void Descriptor_Map()
{
  mapDescriptor["BRISK"] = BRISK;
  mapDescriptor["BRIEF"] = BRIEF;
  mapDescriptor["ORB"] = ORB;
  mapDescriptor["FREAK"] = FREAK;
  mapDescriptor["AKAZE"] = AKAZE;
  mapDescriptor["SIFT"] = SIFT;    
}
                      ...
                      std::vector<cv::DMatch> &matches, std::string descriptorType, std::string matcherType, std::string selectorType)
{
    ...
    if (matcherType.compare("MAT_BF") == 0)
    {
        ...
        cout<<"Brute Force Matching with crossCheck ="<<crossCheck;
    }
...
{
  int threshold = 100;       // difference between intensity of the central pixel and pixels of a circle around this pixel
  bool bNMS = true;
